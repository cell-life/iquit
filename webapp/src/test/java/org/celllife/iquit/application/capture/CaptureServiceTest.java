...
public class CaptureServiceTest {
	...
	public void testConvertXml() throws Exception {
		...
		parameterMap.put("one", "A B C");
		parameterMap.put("two", "D");

		CaptureContext context = new CaptureContext(formName, formId, formBinding);
		String xml = captureServiceImpl.createRequestXml(context, parameterMap);
		Assert.assertEquals(
				"<FormData><data>"
						...
						+ "&lt;two&gt;D&lt;/two&gt;" + "&lt;one&gt;A B C&lt;/one&gt;"
						+ "&lt;/example_study_iquit_form_v1&gt;" + "</data></FormData>", xml);
	}
